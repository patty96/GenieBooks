...
//	 결제하기 버튼 클릭 시 ---->  수민이의 사은품 선택창으로 넘어감 
	 /* function order(){//결제하기 버튼 클릭시
		 if ( "${empty sessionScope.user}" ){//비회원일때
			 document.location.href="http://localhost:8080/bookstore/soomin/order/orderStep0.do?nonMemberbook_no=${bookVo.book_no}&store_code"
					 +store_code;
			 //비회원이면 nonmemberbook_no , store_code
		 }else {//회원일때 
			 document.location.href="http://localhost:8080/bookstore/soomin/order/orderStep0.do?orderCart_idx=${bookVo.book_no}&store_code"
				 +store_code;
